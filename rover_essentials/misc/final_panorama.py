    ...
    try:
        ...
        while True:
            ...
            if duty==2.5:
                sleep(0.8)
            '''
            kit.servo[15].angle=angle_to_point
            sleep(0.2)
            if panorama_ready:
...
def setup_camera(frame,wait_time):
   ...
   # cv2.namedWindow("Webcam")
    ...
    print(len(imgs))
    panorama_ready=True
    stitcher = Stitcher(detector="sift", confidence_threshold=0.99)
...
if __name__ == '__main__':
    servo_thread = Panorama(50,0.05,True)
    camera_thread = Panorama(50,0.05,False)
